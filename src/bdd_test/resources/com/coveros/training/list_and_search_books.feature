Feature: Librarians may list and search books
    ...
    Scenario: Can search a book by id
        Given a book, "The DevOps Handbook", with id of 1, is currently registered in the system
        When a librarian searches by that id
        Then the system returns its full data

    Scenario: Receive an appropriate message if search by id finds nothing
    ...
    Scenario: Receive an appropriate message if search by title finds nothing
        Given no books are registered in the system
